...
const userCreate = async (req, res) => {
    ...
    let userRes = await userModel.findOne({ userEmail });
    ...
    if (!userRes) {
        try {
            ...
            res.send({ status: 0, msg: "Something Went wrong, try a different Email!", error });
        }
    } else {
...
const otpVerify = async (req, res) => {
    ...
    if (storeOtp === otpValue) {
        try {
            const dbPassword = await bcrypt.hash(userPassword, saltRounds);
            ...
            res.send({ status: 0, msg: "Something went wrong, try again later", error });
        }
    } else {
...
const userLogin = async (req, res) => {
    ...
    if (userData) {
        ...
        if (isMatch) {
            ...
            res.send({ status: 2, msg: "Invalid Password" });
        }
    } else {
        res.send({ status: 3, msg: "User not found, Please register !" });
    }
};

export { userCreate, otpVerify, userLogin };
