...
import { freeResPageModel } from "../../models/freeResPageContentModel.js";

export const viewAllFreeRes = async (req, res) => {
  try {
    const result = await freeResModel.find();
    if (!result)
      return res.send({ status: -1, msg: "free resources not found" });
    res.send({
      status: 1,
...
export const viewFreeResPageContent = async (req, res) => {
  try {
    const result = await freeResPageModel.find();
    console.log(result)
    if (!result)
      return res.send({ status: 0, msg: "Cannot found free res page content" });
    else {
      res.send({
        status: 1,
        msg: "free res page content",
        result,
      });
    }
  } catch (error) {
